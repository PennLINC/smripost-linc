...
.. warning::
   ...
   sMRIPost-LINC currently only generates denoised data in MNI152NLin6Asym space.

Defining standard and nonstandard spaces where data will be resampled
=====================================================================
The command line interface of *sMRIPost-LINC* allows resampling the preprocessed data
...
.. important::
   *sMRIPost-LINC* will reduce the amount of output spaces to just spaces listed in ``--output-spaces``,
...
that allows end-user applications such as *sMRIPost-LINC* to flexibly query and pull
template and atlas information.
In other words, *TemplateFlow* enables *sMRIPost-LINC* to access a wide range
of templates (and also custom templates, see below).
Therefore, *TemplateFlow* is central to define *sMRIPost-LINC*'s interface regarding
...
Standard spaces
"""""""""""""""
When using *sMRIPost-LINC* in a workflow that will investigate effects that span across
...
For instance, to instruct *sMRIPost-LINC* to use the MNI template brain distributed with
FSL as coordinate reference the option will read as follows: ``--output-spaces MNI152NLin6Asym``.
By default, *sMRIPost-LINC* uses ``MNI152NLin6Asym`` as spatial-standardization reference.
...
Therefore, *sMRIPost-LINC* will run nonlinear registration processes against the template
T1w image corresponding to all the standard spaces supplied with the argument
``--output-spaces``.
By default, *sMRIPost-LINC* will resample the preprocessed data on those spaces (labeling the
...
""""""""""""""""""""""
To make your custom templates visible by *sMRIPost-LINC*, and usable via
...
* ``fsnative``: similarly to the ``anat`` space for volumetric references,
  including the ``fsnative`` space will instruct *sMRIPost-LINC* to sample the
...
* ``func``, ``bold``, ``run``, ``boldref`` or ``sbref`` can be used to
  ...
  **additional nonstandard spaces** are currently being discussed
  `here <https://github.com/pennlinc/fmriprep/issues/1604>`__.
...
In other words, running *sMRIPost-LINC* with ``--output-spaces MNI152NLin6Asym:res-2
