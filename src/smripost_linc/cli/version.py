...
def check_latest():
    ...
    cachefile = Path.home() / '.cache' / 'smripost_linc' / 'latest'
    try:
    ...
    if latest is None or outdated is True:
        ...
        with suppress(Exception):
            response = requests.get(url='https://pypi.org/pypi/smripost_linc/json', timeout=1.0)
...
def is_flagged():
    ...
    # https://raw.githubusercontent.com/pennlinc/smripost_linc/main/.versions.json
    flagged = ()
    response = None
    with suppress(Exception):
