...
def main():
    ...
    from smripost_linc.cli.parser import parse_args
    from smripost_linc.cli.workflow import build_workflow
    ...
    if 'pdb' in config.execution.debug:
        from smripost_linc.utils.debug import setup_exceptionhook
    ...
    if not config.execution.notrack and not config.execution.debug:
        ...
        from smripost_linc.utils.telemetry import sentry_setup, setup_migas
    ...
    smripost_linc_wf = retval.get('workflow', None)

    # CRITICAL Load the config from the file. This is necessary because the ``build_workflow``
    # function executed constrained in a process may change the config (and thus the global
    # state of sMRIPost-LINC).
    config.load(config_file)

    if config.execution.reports_only:
        sys.exit(int(EXITCODE > 0))

    if smripost_linc_wf and config.execution.write_graph:
        smripost_linc_wf.write_graph(graph2use='colored', format='svg', simple_form=True)

    EXITCODE = EXITCODE or (smripost_linc_wf is None) * EX_SOFTWARE
    if EXITCODE != 0:
        sys.exit(EXITCODE)

    # Generate boilerplate
    with Manager() as mgr:
        from smripost_linc.cli.workflow import build_boilerplate

        p = Process(target=build_boilerplate, args=(str(config_file), smripost_linc_wf))
        p.start()
        p.join()
    ...
    if sentry_sdk is not None:
        ...
        sentry_sdk.add_breadcrumb(message='sMRIPost-LINC started', level='info')
        sentry_sdk.capture_message('sMRIPost-LINC started', level='info')

    config.loggers.workflow.log(
        15,
        '\n'.join(['sMRIPost-LINC config:'] + [f'\t\t{s}' for s in config.dumps().splitlines()]),
    )
    config.loggers.workflow.log(25, 'sMRIPost-LINC started!')
    errno = 1  # Default is error exit unless otherwise set
    try:
        smripost_linc_wf.run(**config.nipype.get_plugin())
    except Exception as e:
        if not config.execution.notrack:
            from smripost_linc.utils.telemetry import process_crashfile
        ...
        config.loggers.workflow.critical('sMRIPost-LINC failed: %s', e)
        raise

    else:
        config.loggers.workflow.log(25, 'sMRIPost-LINC finished successfully!')
        if sentry_sdk is not None:
            success_message = 'sMRIPost-LINC finished without errors'
        ...
        if boiler_file.exists():
            ...
            config.loggers.workflow.log(
                25,
                'Works derived from this sMRIPost-LINC execution should include the '
        ...
        from smripost_linc.reports.core import generate_reports
...
def migas_exit() -> None:
    ...
    from smripost_linc.utils.telemetry import send_breadcrumb
...
if __name__ == '__main__':
    raise RuntimeError(
        'smripost_linc/cli/run.py should not be run directly;\n'
        'Please `pip install` smripost_linc and use the `smripost_linc` command'
    )
