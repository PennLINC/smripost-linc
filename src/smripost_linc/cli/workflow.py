...
``multiprocessing.Process`` that allows smripost_linc to enforce
...
def build_workflow(config_file, retval):
    ...
    banner = [f'Running sMRIPost-LINC version {version}']
    notice_path = Path(pkgrf('smripost_linc', 'data/NOTICE'))
    if notice_path.exists():
        banner[0] += '\n'
        banner += [f"License NOTICE {'#' * 50}"]
        banner += [f'sMRIPost-LINC {version}']
    ...
    # Build main workflow
    init_msg = [
        "Building sMRIPost-LINC's workflow:",
    ...
    retval['workflow'] = init_smripost_linc_wf()

    # Check workflow for missing commands
    missing = check_deps(retval['workflow'])
    if missing:
        build_log.critical(
            'Cannot run sMRIPost-LINC. Missing dependencies:%s',
    ...
    build_log.info(
        'sMRIPost-LINC workflow graph with %d nodes built successfully.',
...
def build_boilerplate(config_file, workflow):
    ...
    if not config.execution.md_only_boilerplate and citation_files['md'].exists():
        ...
        bib_text = Path(pkgrf('smripost_linc', 'data/boilerplate.bib')).read_text()
        citation_files['bib'].write_text(
            bib_text.replace(
                'sMRIPost-LINC <version>',
                f'sMRIPost-LINC {config.environment.version}',
            )
        )
        ...
        cmd = [
            ...
            '--citeproc',
            '--metadata',
            'pagetitle="sMRIPost-LINC citation boilerplate"',
