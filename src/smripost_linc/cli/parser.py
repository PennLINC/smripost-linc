...
def _build_parser(**kwargs):
    ...
    from smripost_linc.cli.version import check_latest, is_flagged
    ...
    verstr = f'sMRIPost-LINC v{config.environment.version}'
    currentv = Version(config.environment.version)
    is_release = not any((currentv.is_devrelease, currentv.is_prerelease, currentv.is_postrelease))

    parser = ArgumentParser(
        description=(
            f'sMRIPost-LINC: fMRI POSTprocessing AROMA workflow v{config.environment.version}'
        ),
    ...
    parser.add_argument(
        ...
        help=(
            ...
            'sMRIPost-LINC (see BIDS-Apps specification).'
        ),
    )
    ...
    g_perfm.add_argument(
        ...
        type=_to_gb,
        metavar='MEMORY_MB',
        help='Upper bound memory limit for sMRIPost-LINC processes',
    )
    g_perfm.add_argument(
        '--low-mem',
        action='store_true',
    ...
    g_other.add_argument(
        ...
        'recommended when running concurrent processes of sMRIPost-LINC.',
    )
    g_other.add_argument(
        '--resource-monitor',
        action='store_true',
    ...
    if latest is not None and currentv < latest:
        print(
            f"""\
You are using sMRIPost-LINC-{currentv},
and a newer version of sMRIPost-LINC is available: {latest}.
...
https://fmriprep.readthedocs.io/en/latest/faq.html#upgrading""",
    ...
    if _blist[0]:
        ...
        print(
            f"""\
WARNING: Version {config.environment.version} of sMRIPost-LINC (current) has been FLAGGED
(reason: {_reason}).
...
def parse_args(args=None, namespace=None):
    ...
    if opts.clean_workdir and work_dir.exists():
        ...
        build_log.info(f'Clearing previous sMRIPost-LINC working directory: {work_dir}')
    ...
    if output_dir == bids_dir:
        parser.error(
            ...
            f"(suggestion: {bids_dir / 'derivatives' / 'smripost_linc-' + version.split('+')[0]}."
        )
