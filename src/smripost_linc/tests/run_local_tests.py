...
def run_tests(test_regex, test_mark):
    ...
    mounted_code = '/usr/local/miniconda/lib/python3.11/site-packages/smripost_linc'
    run_str = 'docker run --rm -ti '
    run_str += f'-v {local_patch}:{mounted_code} '
    run_str += '--entrypoint pytest '
    run_str += 'pennlinc/smripost_linc:main '
    run_str += (
        f'{mounted_code}/smripost_linc '
        f'--data_dir={mounted_code}/smripost_linc/tests/test_data '
        f'--output_dir={mounted_code}/smripost_linc/tests/pytests/out '
        f'--working_dir={mounted_code}/smripost_linc/tests/pytests/work '
    )
    if test_regex:
