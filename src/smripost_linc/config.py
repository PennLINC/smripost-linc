...
The module has a :py:func:`~smripost_linc.config.to_filename` function to allow writing out
the settings to hard disk in *ToML* format, which looks like:

.. literalinclude:: ../smripost_linc/data/tests/config.toml
   :language: toml
   :name: smripost_linc.toml
   :caption: **Example file representation of sMRIPost-LINC settings**.

This config file is used to pass the settings across processes,
using the :py:func:`~smripost_linc.config.load` function.
...
CONFIG_FILENAME = 'smripost_linc.toml'
...
if not any(
    ...
    # https://github.com/pennlinc/smripost_linc/pull/2080#discussion_r409118765
    from fmriprep._warnings import logging
else:
    import logging
...
if os.getenv('IS_DOCKER_8395080871'):
    ...
    if _cgroup.exists() and 'docker' in _cgroup.read_text():
        ...
        _exec_env = 'smripost_linc-docker' if _docker_ver else 'docker'
    del _cgroup
...
class environment(_Config):
    ...
    execution environment, nipype and *sMRIPost-LINC* versions, etc.).
...
def load(filename, skip=None, init=True):
    ...
    filename : :py:class:`os.PathLike`
        TOML file containing sMRIPost-LINC configuration.
    skip : dict or None
